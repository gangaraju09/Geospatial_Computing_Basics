# program to create instances to store the data in a container and creating methods to calculate distance between cities and population change between years
# to check the file is situdated or not and if files is not exit then to exit the program, sys library is used and to calculate some special mathematical formule, math library is used
import sys
import math

# create a class with a name as City
class City(object):
...
# define method to calculate distance between two cities based on their latitudes and logitudes
    def printDistance(self, othercity):
        
# use try and except to avoid bad inputs and to avoid unessary program bombs
        try: 
...
# define a method to calculate population change of city between two years                
...
# used append to input each city instance into the city list 
